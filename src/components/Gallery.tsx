...
export function Gallery() {
  ...
  const shown = galleryImages.slice(0, displayCount);
  ...
  return (
    <section id="gallery" className="py-24 lg:py-32 bg-slate-50 dark:bg-slate-900 relative">
      <div ref={ref} className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="columns-2 md:columns-3 lg:columns-4 gap-4 [&>*]:mb-4">
          {shown.map((image, index) => (
            ...
            >
              <div className="relative rounded-xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 bg-slate-200 dark:bg-slate-800">
                <img
                  src={image.imageSrc}
        ...
        {hasMore && (
          <div className="text-center mt-12">
            <motion.button
              onClick={() =>
                setDisplayCount((p) => Math.min(p + LOAD_MORE_COUNT, galleryImages.length))
              }
              ...
              whileTap={{ scale: 0.95 }}
            >
              {t.gallery.loadMore} ({galleryImages.length - displayCount} {t.gallery.remaining})
            </motion.button>
          </div>
        )}
      </div>
      ...
      <Lightbox
        open={selected !== null}
        imageSrc={selected !== null ? shown[selected]?.imageSrc : undefined}
